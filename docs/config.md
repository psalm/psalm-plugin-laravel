...
**On this branch (Psalm 6):** `TaintedLlmPrompt` is a Psalm 7 core issue type with no Psalm 6 equivalent, so the D-in finding reports as the generic `TaintedCustom` issue instead (shared with this plugin's other custom sink, `html_url`, see [Security checks](security.md)) — message `Detected tainted llm_prompt` rather than a dedicated one. Detection itself is unaffected: the plugin widens ordinary "any untrusted input" sources with the `llm_prompt` kind so the existing sink stubs match (see `LlmPromptTaintBridge`). `findPromptInjection=false` still fully disables it, by skipping that widening rather than suppressing `TaintedCustom` wholesale — suppressing `TaintedCustom` directly would also silence `html_url`, and an explicit `<TaintedLlmPrompt errorLevel="..." />` in `issueHandlers` is rejected outright (not a recognized element) on this branch.
...
A key read by a template reached through `@include` or `@extends` counts as read, because those directives inherit the including template's whole scope. The chain is followed as far as every include in it names a literal template; one dynamic `@include($name)` at any depth silences the check for that call site alone, not for the run.
