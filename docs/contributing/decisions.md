...
## Contract Method Bridging

### Dynamic container resolution over allow-list

**Decision:** `ContractMethodBridgeHandler` bridges a Laravel service's public concrete-only methods onto the `Illuminate\Contracts\*` interface it implements by dynamically walking every scanned `Illuminate\Contracts\*` interface (minus a small exclusion list, see below) and resolving each one's concrete via the booted container (`$container->make($contract)`), instead of matching against a hand-maintained allow-list of contract-to-concrete pairs.

This supersedes the handler's original allow-list, which listed only `Contracts\Foundation\Application => Foundation\Application` and excluded everything else on principle. That caused a class of false positives (#1230: `Cache::driver()->flexible(...)` raised `UndefinedInterfaceMethod` because `CacheManager::driver()` returns the contract, but `flexible()` lives only on the concrete `Illuminate\Cache\Repository`) for every other contract with a single, driver-invariant concrete — `Cache\Repository` included, since every cache driver constructs the same `Illuminate\Cache\Repository` class and only the wrapped `Store` differs. The dynamic walk mirrors Larastan's `ContractsMethodsExtension` in mechanism (same `make()` + `Throwable`-guard resolution), not in trigger: Larastan resolves a contract lazily, only when a method lookup on it actually misses; this walk resolves every scanned contract eagerly at population time. Eager is acceptable because Laravel service construction is lazy about I/O regardless (e.g. a Redis-backed store's constructor stores the connection manager; the TCP handshake happens on first operation, not construction), so eager resolution costs object construction only, and a contract whose `make()` throws is silently skipped exactly as a lazy resolution would be.

**Why:** An allow-list requires a maintainer to notice and add every affected contract one issue at a time. The container already knows the true binding for any contract at analysis time (`registerCoreContainerAliases()` plus whatever the app itself rebinds) — asking it directly is both more general and more accurate than a static table, and it degrades gracefully: `make()` throwing (unbound abstract, partial boot, provider error) or returning a non-object both resolve to "don't bridge," so a contract the container can't fulfil is left alone exactly as before.

**Exclusion list, kept:** `Illuminate\Contracts\Filesystem\Filesystem`, `Filesystem\Cloud`, and `Queue\Queue` are excluded (`ContractMethodBridgeHandler::EXCLUDED_CONTRACTS`) because for these contracts the resolved concrete CLASS itself — not just a wrapped dependency — varies by configured driver: `local`/`ftp`/`sftp` disks resolve `FilesystemAdapter`, `s3` resolves `AwsS3V3Adapter extends FilesystemAdapter` with driver-only publics (`url()`, `temporaryUrl()`, `temporaryUploadUrl()`, `getClient()`); `sync`/`database`/`redis`/`sqs` queue connections resolve `SyncQueue`/`DatabaseQueue`/`RedisQueue`/`SqsQueue`, each with driver-specific publics. Bridging the default driver's surface here would lie at the class-hierarchy level for every other driver — a stronger and less defensible claim than the accepted per-driver trade-off below. `StorageHandler` already encodes the correct, narrower policy for this: it narrows only the RETURN of `disk()`/`drive()` to the concrete adapter, never the `Filesystem`/`Cloud` contract itself.

**Trade-off, accepted, for contracts that ARE bridged (e.g. `Cache\Repository`), where the concrete CLASS is invariant across drivers:** the bridge reflects the DEFAULT binding's concrete, resolved once at population time. A call against a non-default driver (`Cache::driver('redis')` when the default is `file`) can surface methods the default driver's concrete happens to have but the other driver's concrete lacks, or vice versa. Accepted because it matches Larastan's own behavior for the same contracts, the default binding is the dominant runtime shape for most apps, and an app that rebinds a contract to a custom implementation bridges to ITS concrete (resolved per-app, not a fixed mapping) — custom implementations are represented truthfully instead of assumed away.

**Invariant, unchanged by this pivot:** the handler still writes only `declaring_method_ids` / `appearing_method_ids` on the contract's `ClassLikeStorage`, never `methods[]` — `UnimplementedInterfaceMethod`'s compliance check iterates `methods[]`, so writing to it would force every implementor to declare the bridged methods. See `ContractMethodBridgeHandler`'s class docblock for the full mechanism (magic-method skip, visibility gate, hook choice).

**Ripple effect fixed alongside this pivot:** bridging `Illuminate\Contracts\Auth\Guard` / `StatefulGuard` (previously excluded by the allow-list) turns `authenticate()`, `getUser()`, `getLastAttempted()`, and `logoutOtherDevices()` into real (mixin-resolved) methods on `AuthManager`, which routes past `AuthMethodHandler`'s missing-method-path narrowing and collapses the return type from the config-resolved concrete user model down to the generic `Authenticatable` contract. Fixed by registering `Guard`/`StatefulGuard` directly on `AuthMethodHandler` (see that class's docblock) so the narrowing applies regardless of which path Psalm takes to resolve the call. This class of ripple — a bridged contract changing behavior for an unrelated class that `@mixin`s it — is a systemic risk of the dynamic walk that the allow-list never had to consider; watch for it when a future contract addition interacts with an existing `@mixin`-based handler.

**Registration-order constraint introduced by the same ripple:** once `AuthMethodHandler` is registered for `Guard`/`StatefulGuard` (needed for the fix above), it and `GuardHandler` both answer `user()`/`loginUsingId()`/`onceUsingId()` on those two interfaces. Psalm's `MethodReturnTypeProvider` dispatch returns the first non-null answer in registration order (`Plugin::registerHandlers()`), and `AuthMethodHandler`'s answer for those three methods is an unconditional default-guard narrowing with no awareness of a `->guard('name')` receiver chain — weaker than `GuardHandler`'s chain-aware logic. `GuardHandler` is therefore registered BEFORE `AuthMethodHandler`; swapping that order silently collapses every `->guard('name')->user()` call to the default guard's model. `AuthMethodHandler` still exclusively owns `authenticate()`/`getUser()`/`getLastAttempted()`/`logoutOtherDevices()`, which `GuardHandler` never answers, so ordering doesn't affect those.

**Applies to:** #1230, #1108.
