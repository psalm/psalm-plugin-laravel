...
**Decision:** `ContractMethodBridgeHandler` bridges concrete-only public methods onto every scanned `Illuminate\Contracts\*` interface by resolving each one's concrete via the booted container (`make()` in a `Throwable` guard; failures skip), replacing the original single-entry allow-list (`Foundation\Application` only). Mirrors Larastan's `ContractsMethodsExtension`, but eager at population instead of lazy per method-miss (safe: Laravel constructors defer I/O). Fixes the #1230 class of false positives (`Cache::driver()->flexible()`); the container already knows the true binding, including app rebinds, so asking it beats maintaining a table.

**Exclusions** (`EXCLUDED_CONTRACTS`): `Filesystem\Filesystem`, `Filesystem\Cloud`, `Queue\Queue` — their resolved concrete CLASS varies by configured driver (`s3` → `AwsS3V3Adapter`; queue drivers → `RedisQueue`/`SqsQueue`/...), so bridging the default driver's surface would lie for the others. `Cache\Repository` is bridged: every driver constructs the same class. Accepted residual trade-off for bridged contracts: the default binding's surface is assumed (same as Larastan).

**Invariant:** write only `declaring_method_ids`/`appearing_method_ids`, never `methods[]` (`UnimplementedInterfaceMethod` iterates it and would force implementors to declare bridged methods).

**Ripple to watch:** bridging a contract turns `@mixin <contract>` methods on other classes into real methods, so Psalm dispatches by the mixin target and bypasses providers registered only on the mixing class. Fixed for Auth by registering `AuthMethodHandler` on Guard/StatefulGuard, which then required `GuardHandler` to register first (Psalm takes the first non-null provider answer in registration order; GuardHandler's chain-aware `->guard('name')` logic must beat the default-guard fallback). Check `@mixin \Illuminate\Contracts\*` interactions whenever a handler gains contract registrations.
