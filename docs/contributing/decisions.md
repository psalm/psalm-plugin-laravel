...
**Widening the proof without widening the risk (#1416):** the initial #1348 proof required an array literal at the call site with every entry a literal string pair, which cleared 1 of 20 real `response()->make()` call sites on a Laravel 13 codebase surveyed for the follow-up. Four widenings closed most of that gap, each keeping the "fail toward a retained finding" invariant on every new branch:

- **Local variable const-fold.** A `$headers` variable is resolved to its array literal only when the enclosing function-like contains exactly two occurrences of that name (the single `$headers = [...]` assignment before the call, and the call argument itself), with `ClosureUse` counted alongside `Variable` so a captured variable inflates the count and disqualifies. A function-like-wide scan for variable-variables (`$$x`, `${expr}`) and `extract()`/`compact()`/`get_defined_vars()` bails independently of that count, because none of the three produces a `Variable` node the count could see. No enclosing function-like (top-level code) keeps the sink outright: there is no scope to prove a single assignment within.
- **Interpolated/concatenated disposition prefix.** `"attachment; filename=\"{$name}.csv\""` and `'attachment; filename=' . $name` are accepted when the literal leading part (the first `InterpolatedStringPart`, or the leftmost leaf of a left-associative `Concat` chain) already matches `/^attachment\s*;/i` and passes the existing control-character guard. The interpolated or concatenated suffix is never inspected: nothing that can appear after a literal parameter separator can retract the `attachment` token. A prefix split across two concatenated string literals (`"attachment" . "; filename=" . $name`) is deliberately not reassembled — only the single leftmost leaf carries the proof.
- **`Illuminate\Http\Response::__construct` as a second call route.** Confirmed empirically (dumping `TaintedInput::$journey` tails) to yield the same tail shape as `make()`: label `call to Illuminate\Http\Response::__construct`, location spanning the `$content` argument. Added as a fourth `SINK_CALL_LABELS` entry and a `New_` branch in the call matcher, with no class-name gate on the `New_` node — the journey label already proves Psalm resolved the call to this specific stubbed sink, the same trust already extended to `parent::make()` / `$this->make()` / `static::make()` receivers.
- **Literal `Content-Type` denylist.** The issue proposed a whitelist of media types known not to render as HTML; that was rejected for a denylist instead: a literal, well-formed media type (`RFC 6838`-shaped token pair, parameters after the first `;` discarded) is exempt unless it contains `html` or `xml` anywhere (covers `text/html`, `application/xhtml+xml`, every `*+xml` suffix including `application/xml`, and `image/svg+xml` — XML can carry an XHTML-namespaced `<script>`) or starts with `multipart/`. A denylist keeps every future, unenumerated vendor download type (`application/vnd.*`, ...) exempt without a maintenance list, at the cost of leaving genuinely-safe XML-suffixed types on the sink; that tradeoff was chosen deliberately over the narrower, harder-to-maintain whitelist. The denylist's safety premise is that a browser renders a declared type as-is; the WHATWG MIME Sniffing spec's one exception (a supplied essence of `unknown/unknown`, `application/unknown`, or `*/*` can still sniff to `text/html`) is excluded by name rather than relying on the `html`/`xml` needles to catch it. A third needle, `script`, closes the WHATWG JavaScript MIME group (a response an attacker controls is as dangerous loaded as a `<script>` as it is rendered as HTML); its one known false-positive collateral, `application/postscript` keeping the sink, is accepted as the correct failure direction.

The two proofs (attachment disposition, safe content type) run in one pass over the header entries so a duplicate fold of either header name is caught regardless of which proof would otherwise succeed. An entry that is not itself a candidate for either proof only needs a literal string key (so a duplicate fold can still be detected); its value may be any expression, since it cannot retract a proof made by a different entry.
