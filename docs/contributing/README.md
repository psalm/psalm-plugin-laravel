...
- The contract is persisted as the manifest's sixth slot, so a template fresh enough to skip recompiling still has one. `ShadowManifest::normalizeEntries()` drops any entry that is not six slots wide, which self-evicts everything written before contracts existed at the cost of one recompile.
