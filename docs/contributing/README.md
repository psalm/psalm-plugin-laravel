...
2. **Closure-level** (model property and method handlers): register via `$providers->property_type_provider->registerClosure(...)` or `$methods->return_type_provider->registerClosure(...)` — used by `ModelRegistrationHandler` to bind property and method handlers per-model after codebase is populated. Some closure-level handlers are additionally gated behind `<experimental>` (see [Configuration](../config.md#experimental)) instead of a plain config flag. `ModelToArrayShapeHandler` is the first example: it registers on `$methods->return_type_provider`, and `ModelRegistrationHandler::setExperimentalFeatures()` receives the resolved experimental feature list, whose membership guards its `registerClosure()` call, mirroring the existing `enableMigrations()` pattern used for `columnFallback=migrations`.

### Experimental features

Rules or handlers that are not yet stable enough for every user to run unconditionally are gated behind `<experimental>` (see [Configuration](../config.md#experimental)) instead of a plain on/off config flag. The registry lives in `src/Config/ExperimentalFeature.php`. Two mutually-exclusive forms: granular opt-in, `<experimental><feature name="..." /></experimental>` (enable named features only), or `<experimental all="true" />` (enable every experimental feature), optionally narrowed with `<exclude name="..." />` children for features that misbehave on a codebase.

Lifecycle:

1. **Introduce**: add a case to `ExperimentalFeature`, default off. Gate the registration on `PluginConfig::isExperimentEnabled(ExperimentalFeature::YourCase)`. Note the exposure asymmetry: a new case becomes active on upgrade for everyone running `all="true"`, but granular `<feature>` users stay unaffected until they add its name. Expect broader exposure (and more incoming bug reports) from the `all="true"` cohort, and treat `<exclude>` as their escape hatch. Document the feature under [Configuration](../config.md#experimental) and note its experimental status in the handler's docblock.
2. **Stabilize**: remove the case from `ExperimentalFeature`, add its name to the `RETIRED` map with the full graduated notice (the exact sentence the user reads, e.g. `"Experimental feature 'x' graduated to stable in v4.16 and no longer needs <experimental>. Remove it from psalm.xml."`), and make its registration unconditional. A leftover `<feature name="x">` or `<exclude name="x">` for the graduated feature then produces that notice instead of an error, so an existing psalm.xml keeps working until the user removes the entry. Keep the entry for at least one minor release, then drop it at the next major.
3. **Withdraw**: same mechanics, but the `RETIRED` notice explains the feature was withdrawn (with a short reason) instead of graduated, and the registration is not made unconditional. Used when a feature turns out to be the wrong approach rather than simply not yet stable.
