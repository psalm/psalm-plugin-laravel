...
Same shape is used on `Promptable::stream()`, `queue()`, `broadcast*()`, the `\Laravel\Ai\agent()` factory, `AgentPrompt::prepend()`/`append()`/`revise()`, `Files\Document::fromString()`/`fromBase64()`, `PendingReranking::rerank()`, `Messages\UserMessage`, and `Messages\Message::__construct()`.
