...
A queued class that ends up with a `__serialize()` or `__sleep()`, whether from `SerializesModels` (directly, through a parent, or through a trait that itself uses it), from another trait, or hand-written. That covers `Illuminate\Foundation\Queue\Queueable`, which `make:job` scaffolds since Laravel 11, and `Illuminate\Notifications\Notification`.

Also silent: abstract classes, static properties, and properties inherited from a parent or declared by a trait.

Known blind spots, all in the safe direction:

- models held in a plain `array`, and `Support\Collection` of models (not a `QueueableCollection`, so the trait would not convert it either)
- queued event listeners, whose own properties never enter the payload: Laravel queues a `CallQueuedListener` wrapper and re-resolves the listener from the container
