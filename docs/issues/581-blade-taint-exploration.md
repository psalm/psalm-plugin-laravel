...
| PR-6b (TBD) | Component data flow (`<x-foo :bar="$data" />`). Local x-tag scanner records component edges into `BladeSafetyMap` mirroring PR-4's include-edge propagation; Laravel's `ComponentTagCompiler` reuse rejected during PR-5 (per-tag try/catch isolation would re-implement the tag loop). | not started |
