...
| PR-5 [#985](https://github.com/psalm/psalm-plugin-laravel/pull/985) | Refactor only: extracts the view-chain receiver-walk out of `BladeAwareViewTaintHandler` into a dedicated `ReceiverViewNameResolver` pure-AST utility. No behaviour change. Adds 26 direct AST-level unit tests covering the resolution table. Decouples the receiver-walk from the dispatcher so PR-6 can extend the chain-preserving allowlist (Mailable view/markdown/text chains) without touching taint sink construction. | merged into umbrella |
| PR-6a (this PR) | Mailable `view()`/`markdown()`/`text()` -> `with()` chain via two new `ReceiverViewNameResolver` parameters: `$extraViewBinders` (binder method names that record arg[0] AND recurse) and `$recurseThroughUnknownMethods` (treat any other method as chain-preserving). `WithLikeMethodSpec` carries both; the dispatcher passes them straight through. Mailable's chain interleaves view-binders with decorators (`->subject()`, `->from()`, `->locale()`, etc.) — the recurse-through-unknowns flag is what makes the upstream `view()` visible from the `with()` site. Multi-binder chains (`view('a')->text('b')->with(...)`) refuse, mirroring multi-literal `View::first(['a','b'])`. `MailMessage::with` is intentionally NOT registered (single-arg "append notification line" semantic from `SimpleMessage`, would mis-dispatch; pinned by a dedicated regression test). | shipping |
| PR-6b (this PR) | Anonymous-component data flow (`<x-foo :bar="$data" />`). A local x-tag scanner (`BladeComponentTagParser`) extracts self-closing component tags into `BladeComponentEdge` records that carry Laravel's three anonymous-component candidate view names (`components.{name}`, `components.{name}.index`, `components.{name}.{last-segment}`); kebab-case and snake_case attribute names are camelized to match the child template's scope-local variable names. `BladeSafetyMap::build()` extends the existing fixed-point pass with a component-edge propagation rule that, unlike `@include`, has NO mergeData pass-through: a child unsafe key contributes to the parent only when the parent binds it via an attribute. Cycle detection is unified across both edge types so a cross-kind cycle (`A` includes `B`, `B` uses `<x-A />`) is detected. Out of scope for v1 and forced to `ComponentTag` UNKNOWN: opening tags with bodies (slot content), namespaced anonymous components (`<x-mail::message>`), `<x-dynamic-component>`, the legacy `@component(...)` / `@slot(...)` directive forms, class components, and attribute strings containing `@class(...)` / `@style(...)` / `{{ $attributes }}` / variable-variable expressions. | shipping |
| PR-6c (TBD) | `View::share()` global view data — runtime-injected across every `view()` call from any caller; collect share keys during scan and union them onto every UNSAFE_KEYS / UNKNOWN dispatch. | not started |
| PR-6d (TBD) | Scanner result caching across analysis runs (file-mtime keyed). Defer until 6b/6c stabilise the safety-map shape. | not started |
| Deferred | Variable-bound view-builder chains (`$v = view('home'); $v->with(...)`) — see "Key decisions" below. Also deferred: section graph for `@extends`/`@section`/`@yield`. | not started |
