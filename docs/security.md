...
with this plugin's `html_url` sink — distinguished from that sink by its message,
`Detected tainted llm_prompt`. Detection itself is faithful — see
...
The guard-exemption mechanism below (`@psalm-taint-escape llm_prompt` on a
middleware guard) works the same way on this branch: the handler matches the
`TaintedCustom` finding by that same message instead of `TaintedLlmPrompt`'s
class, so an annotated guard suppresses the `TaintedCustom(llm_prompt)` finding
exactly as it suppresses `TaintedLlmPrompt` on Psalm 7. The `html_url` sink
sharing the same issue class is unaffected, since the message text differs.
...
What it does: the D-in finding (`TaintedLlmPrompt`, or `TaintedCustom` on this branch — see above)
stops being reported for `prompt()` and `stream()` on that agent. Every other taint kind keeps
flowing, so the same value reaching SQL, HTML, or a shell command is still reported.
