...
* `InvalidScope` and `NonStaticSelfCall`, when the message is `Use of $this in non-class context`, `Invalid reference to $this in a non-class context`, or `Cannot use self`/`static outside class context` (whichever keyword and casing the author wrote) (#1559). A Blade shadow is classless global scope, so a plain `$this`, `self::`, or `static::` mention — conventions some component libraries rely on inside their own template markup — trips these checks unconditionally, on every occurrence. The bare-fetch `$this` message is unreachable today, because the prelude declares `@var mixed $this` for any template that mentions the name, satisfying Psalm's own scope check ahead of it; gated anyway so a future prelude change cannot silently regress into flooding noise. Not dropped: `Invalid reference to $this in a static context` and `Method X::y is not static, but is called using self::`, both genuine author mistakes reported only when a `@php class ... @endphp` block compiles a real class into the shadow. With [`reportMixedIssues`](config.md#reportmixedissues) turned on, a `Mixed*` finding can still surface on the same line a scope issue was dropped from, since the receiver of a dropped call stays typed `mixed`.
