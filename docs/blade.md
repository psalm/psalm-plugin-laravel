...
* **A malformed component tag reports a genuine `ParseError`, always on its opening line.** An empty bound attribute (`:value=""`) compiles to a dangling `,` or `]` in the component's data array, and a closing tag with no matching opener (a stray `</x-alert>` over a self-closed `<x-alert />`) compiles to an orphan `@endif`. Both are real bugs Laravel's own compiler would hit at render time, not remap artifacts, and neither has a more precise line to report on than the tag itself, for the same reason a multi-line tag does not: there is no per-attribute marker inside a component tag's compiled output.
