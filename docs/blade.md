...
* `UnusedVariable` and `UnevaluatedCode`, unconditionally, under `findUnusedVariablesAndParams`/`--find-unused-code`. A `<x-...>` tag compiles to a `$__componentOriginal*`/`$__attributesOriginal*` save-and-restore pair around the render, and `@foreach`/`@forelse` reassign `$loop = $__env->getLastLoop();` at the end of the loop; none of that bookkeeping is ever read back, so it reports `UnusedVariable` on every template that uses either construct. A `@switch` arm's `@break` similarly leaves Psalm treating the following `@case`/`@default` line as unreachable, reporting `UnevaluatedCode`. `UnusedForeachValue` is not part of this drop: it fires on the author-named foreach variable, not compiler bookkeeping, and is real signal. Trade-off: an author's own dead store inside `@php` and an unused foreach *key* also report as `UnusedVariable` and are silenced along with the compiler noise.
