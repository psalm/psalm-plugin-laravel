--SKIPIF--
<?php
require getcwd() . '/vendor/autoload.php';
// PromptInjection fixtures need the optional laravel/ai integration installed (the plugin's
// laravel-ai stubs load only when Plugin::optionalIntegrationStubs() sees
// LaravelAiIntegration::isEnabled()); it is not a root composer.json
// dependency (PHP ^8.3 floor would break the PHP 8.2 CI lanes). Skip rather than fail when absent.
if (!\Psalm\LaravelPlugin\Internal\LaravelAiIntegration::isEnabled() || !trait_exists(\Laravel\Ai\Promptable::class)) {
    echo 'skip needs supported laravel/ai package (>=0.11.0 <1.0.0)';
}
--ARGS--
--no-progress --no-diff --config=./tests/Type/psalm.xml --taint-analysis
--FILE--
<?php declare(strict_types=1);

namespace GuardStripped\Guards {
    final class StrippedPromptGuard
    {
        /**
         * @psalm-taint-escape llm_prompt
         */
        public function handle(string $prompt, \Closure $next): mixed
        {
            return $next($prompt);
        }
    }
}

namespace GuardStripped\Agents {
    class StrippedGuardedBase implements \Laravel\Ai\Contracts\HasMiddleware
    {
        use \Laravel\Ai\Promptable;

        /**
         * @return list<\GuardStripped\Guards\StrippedPromptGuard>
         */
        #[\Override]
        public function middleware(): array
        {
            return [new \GuardStripped\Guards\StrippedPromptGuard()];
        }

        // The journey label names this class, but the runtime receiver may be the subclass below.
        public function ask(string $question): \Laravel\Ai\Responses\AgentResponse
        {
            return $this->prompt($question);
        }
    }

    final class StrippedChildAgent extends StrippedGuardedBase
    {
        /**
         * @return list<never>
         */
        #[\Override]
        public function middleware(): array
        {
            return [];
        }
    }

    function askStripped(\Illuminate\Http\Request $request): \Laravel\Ai\Responses\AgentResponse
    {
        return (new StrippedChildAgent())->ask((string) $request->input('q'));
    }
}

?>
--EXPECTF--
TaintedLlmPrompt on line %d: Detected tainted LLM prompt
