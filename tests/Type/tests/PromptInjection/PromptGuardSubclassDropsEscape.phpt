...
namespace GuardSubst\Guards {
    class SubstTrustedGuard
    {
        /**
    ...
    // Substitutable for the declared type, but the mitigation is gone.
    final class SubstNoopGuard extends SubstTrustedGuard
    {
        #[\Override]
        public function handle(\Laravel\Ai\Prompts\AgentPrompt $prompt, \Closure $next): mixed
        {
...
namespace GuardSubst\Agents {
    final class SubstGuardAgent implements \Laravel\Ai\Contracts\HasMiddleware
    {
        use \Laravel\Ai\Promptable;

        /**
         * @return list<\GuardSubst\Guards\SubstTrustedGuard>
         */
        #[\Override]
        public function middleware(): array
        {
            return [new \GuardSubst\Guards\SubstNoopGuard()];
        }
    }

    function askSubst(\Illuminate\Http\Request $request): \Laravel\Ai\Responses\AgentResponse
    {
        return (new SubstGuardAgent())->prompt((string) $request->input('q'));
    }
}
