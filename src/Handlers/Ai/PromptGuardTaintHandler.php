...
/**
 * Suppresses `TaintedLlmPrompt` (`TaintedCustom` on Psalm 6, see below) on a `prompt()`/`stream()`
 * call whose receiver declares an agent middleware stack containing a guard annotated
 * `@psalm-taint-escape llm_prompt` on its dispatched method. Psalm parses that natively into
 * `MethodStorage::$removed_taints`; nothing here names a specific guard package, so any library or
 * app-local guard opts in with that one docblock line.
 *
 * PSALM 6: this branch has no `TaintedLlmPrompt` issue class or `TaintKind::INPUT_LLM_PROMPT` int
 * constant — {@see \Psalm\LaravelPlugin\Handlers\Ai\LlmPromptTaintBridge} makes the `llm_prompt` D-in
 * finding surface as the generic `TaintedCustom` issue instead, with the message
 * `Detected tainted llm_prompt` (the literal taint kind string, set by
 * `TaintFlowGraph`'s `default => new TaintedCustom('Detected tainted ' . $matching_taint, ...)`
 * branch). Gate 1 below matches on that message rather than an issue class. `$removed_taints` is
 * `list<string>` on Psalm 6, not an int bitmask, so gate 8 checks `in_array('llm_prompt', ..., true)`
 * rather than a bitwise `&`.
 *
 ...
 * 1. Issue is `TaintedLlmPrompt` (Psalm 7) or `TaintedCustom` with the `llm_prompt` message
 *    (Psalm 6 — see PSALM 6 above).
...
final class PromptGuardTaintHandler implements BeforeAddIssueInterface
{
    ...
    /**
     * Psalm 6 has no `TaintedLlmPrompt` issue class; this is the exact message
     * `TaintFlowGraph`'s `TaintedCustom` fallback produces for the `llm_prompt` kind
     * ({@see \Psalm\LaravelPlugin\Handlers\Ai\LlmPromptTaintBridge}).
     */
    private const PSALM6_MESSAGE = 'Detected tainted llm_prompt';

    /** Matches `LlmPromptTaintBridge::LLM_PROMPT` and the sink stubs' `@psalm-taint-sink llm_prompt`. */
    private const LLM_PROMPT_KIND = 'llm_prompt';
    ...
    public static function beforeAddIssue(BeforeAddIssueEvent $event): ?bool
    {
        ...
        if (!$issue instanceof TaintedCustom || $issue->message !== self::PSALM6_MESSAGE) {
            return null;
        }
    ...
    private static function dispatchedMethodEscapes(Codebase $codebase, ClassLikeStorage $guard, bool $asObject): bool
    {
        ...
        foreach ($dispatchOrder as $methodName) {
            ...
            return \in_array(self::LLM_PROMPT_KIND, $dispatched->removed_taints, true);
        }
