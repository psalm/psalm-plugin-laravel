...
# Plugin-deps signature (require + autoload) the cached vendor was solved against.
PLUGIN_SIG_FILE="$APP_SRC/.plugin-dep-sig"

# Cache is keyed on app ref, not plugin, so a vendor from one line (Psalm 7 on 4.x)
# can be restored for another (Psalm 6 on 3.x) and fatal every handler at load
# (`addTaints()` returns array in 6, int in 7). Reinstall on any drift or missing sig.
