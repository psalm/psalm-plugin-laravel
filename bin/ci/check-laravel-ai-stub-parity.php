 ...
 * reflection during analysis (M2 in the design doc: redeclaration is the
 * declarative default), so Psalm itself never notices when a stub's native
 ...
 * (issue #484) rather than fixed here: this script owns detection, not the
 * stub files themselves. Printed as findings either way, never silently
 * hidden, they just don't fail the job. Remove an entry once its stub is
 * fixed, so the checker starts enforcing it like everything else.
 *
...
const KNOWN_GAPS = [
    ...
    'Laravel\Ai\Responses\StreamableAgentResponse::__toString' => 'stub declares this as a taint-source method, but the installed class does not define it at all.',
];
...
foreach (findStubFiles($stubsDir) as $file) {
    ...
    if ($ast === null) {
        report($file, "{$file}: php-parser could not parse this stub", $mismatches, $knownGaps);
        continue;
    }
    ...
    foreach (findClassLikes($ast) as $classLike) {
        ...
        if (!\class_exists($fqcn) && !\interface_exists($fqcn) && !\trait_exists($fqcn)) {
            report($fqcn, "{$fqcn}: declared in {$file} but not found in the installed laravel/ai package (renamed or removed upstream?)", $mismatches, $knownGaps);
            continue;
        }
        ...
        foreach ($classLike->getMethods() as $method) {
            ...
            if (!$reflectionClass->hasMethod($methodName)) {
                report($key, "{$key}(): declared in the stub but not found on the installed class (renamed or removed upstream?)", $mismatches, $knownGaps);
                continue;
            }
    ...
    foreach (findFunctions($ast) as $function) {
        ...
        $comparedMethods++;
        diffSignature($fqcn, $function->params, $function->returnType, new \ReflectionFunction($fqcn), null, $mismatches, $knownGaps);
    }
}
 ...
 */
function report(string $key, string $message, array &$mismatches, array &$knownGaps): void
{
...
function diffSignature(
    ...
    foreach ($stubParams as $position => $stubParam) {
        ...
        if ($stubType !== $vendorType) {
            $paramName = $stubParam->var instanceof Node\Expr\Variable && \is_string($stubParam->var->name)
                ? '$' . $stubParam->var->name
                : "position {$position}";
            report($label, "{$label}({$paramName}): stub says \"{$stubType}\", installed laravel/ai says \"{$vendorType}\"", $mismatches, $knownGaps);
        }
    }
    ...
    if ($stubReturnType !== null) {
        ...
        if ($stubReturn !== $vendorReturn) {
            report($label, "{$label}(): return type stub says \"{$stubReturn}\", installed laravel/ai says \"{$vendorReturn}\"", $mismatches, $knownGaps);
        }
    }
}
